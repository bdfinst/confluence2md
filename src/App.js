...
class App {
  ...
  convert(dirIn, dirOut) {
    ...
    const pages = ((() => {
      const result = [];
      for (let filePath of Array.from(filePaths)) {         if (filePath.endsWith('.html')) {
  ...
  convertPage(page, dirIn, dirOut, pages) {
    ...
    const fullOutFileName = this._path.join(dirOut, page.space, page.fileNameNew);
